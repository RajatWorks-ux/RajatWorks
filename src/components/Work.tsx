...
// ════════════════════════════════════════════════
//  LIGHTBOX  (reused from WorkImage)
// ════════════════════════════════════════════════
const Lightbox = ({
...
const BentoCard = ({
...
}: { project: (typeof config.projects)[0]; index: number; variant: "wide" | "tall" | "square"; }) => {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { el.classList.add("bento-card--visible"); obs.disconnect(); } }, { threshold: 0.08 });
    obs.observe(el);
  ...
  return (
    <>
      <div ref={cardRef} className={`bento-card bento-card--${variant}${expanded ? " bento-card--expanded" : ""}`} style={{ animationDelay:`${index * 80}ms` }}>
        ...
        <div className="bento-info">
          ...
          <div className="bento-desc-block">
            ...
            {expanded && extraDesc && <p className="bento-desc bento-desc--extra">{extraDesc}</p>}
          ...
          <div className="bento-bottom-row">
            ...
            {hasMore && (
              <button className="bento-toggle" onClick={() => setExpanded((v) => !v)}>
                {expanded ? "Less ↑" : "More ↓"}
              </button>
            )}
          </div>
...
// ════════════════════════════════════════════════
//  MOBILE CARD  — exact purana layout
// ════════════════════════════════════════════════
...
// ════════════════════════════════════════════════
//  MAIN WORK SECTION
//  Desktop (>1024px)  → Bento Grid, no GSAP pin
//  Mobile  (≤1024px)  → exact purana vertical layout
// ════════════════════════════════════════════════
const Work = () => {
  ...
  // In-view class for heading + mobile cards animation
  useEffect(() => {
    const el = sectionRef.current; if (!el) return;
    const obs = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { el.classList.add("in-view"); obs.disconnect(); } }, { threshold: 0.05 });
    obs.observe(el);
    return () => obs.disconnect();
  }, []);
